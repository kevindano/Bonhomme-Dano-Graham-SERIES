########################################################################
# Goal: computation of the identified set for the logit case with T=4
# Julia Version 1.5.3
########################################################################
...
function true_conditional_probability_given_alpha_x1(theta,y4,y3,y2,y1,x4,x3,x2,x1,alpha)
        #covariates X are iid Bernouilli(1/2) independent of everything else
        H_alpha = (softmax(theta*x4+alpha)^y4)*((1-softmax(theta*x4+alpha))^(1-y4))*(1/2)
        H_alpha = H_alpha*(softmax(theta*x3+alpha)^y3)*((1-softmax(theta*x3+alpha))^(1-y3))*(1/2)
        H_alpha = H_alpha*(softmax(theta*x2+alpha)^y2)*((1-softmax(theta*x2+alpha))^(1-y2))*(1/2)
        H_alpha = H_alpha*(softmax(theta*x1+alpha)^y1)*((1-softmax(theta*x1+alpha))^(1-y1))
        return H_alpha
end

#Computes probability of a choice history given x1
function true_conditional_probability_given_x1(theta,y4,y3,y2,y1,x4,x3,x2,x1,alpha_grid,pi_alpha)
        H = 0.0
        for (index,alpha) in enumerate(alpha_grid)
                H_alpha = true_conditional_probability_given_alpha_x1(theta,y4,y3,y2,y1,x4,x3,x2,x1,alpha)
...
function true_probabilities(theta,alpha_grid,pi_alpha)
        H = zeros(2,2,2,2,2,2,2,2) #order y4,y3,y2,y1,x4,x3,x2,x1 - 1st entry is for "0" and 2nd is for "1"
        for y4 in 0:1
                for x4 in 0:1
                        for y3 in 0:1
                                for x3 in 0:1
                                        for y2 in 0:1
                                                for x2 in 0:1
                                                        for y1 in 0:1
                                                                for x1 in 0:1
                                                                        H[y4+1,y3+1,y2+1,y1+1,x4+1,x3+1,x2+1,x1+1]=true_conditional_probability_given_x1(theta,y4,y3,y2,y1,x4,x3,x2,x1,alpha_grid,pi_alpha)
                                                                end
                                                        end
                                                end
                                        end
...
for theta in theta_grid
        ...
        set_optimizer_attribute(m,"PrimalTolerance", 10^(-11))
        set_optimizer_attribute(m,"DualTolerance", 10^(-11))
        ...
        #Order of dimensions: y3,y2,y1,x4,x3,x2,x1,alpha
        @variable(m, psi[1:2,1:2,1:2,1:2,1:2,1:2,1:2,1:K])
                                ...
                                for y1 in 0:1
                                        for x4 in 0:1
                                                for x3 in 0:1
                                                        for x2 in 0:1
                                                                for x1 in 0:1
                                                                        temp = map(alpha-> (softmax(theta*x4+alpha)^y4)*(1-softmax(theta*x4+alpha))^(1-y4),alpha_grid)
                                                                        constraint[counter]=@constraint(m, H[y4+1,y3+1,y2+1,y1+1,x4+1,x3+1,x2+1,x1+1]-temp'psi[y3+1,y2+1,y1+1,x4+1,x3+1,x2+1,x1+1,:]==0)
                                                                        counter+=1
                                                                end
                                                        end
                                                end
        ...
        for x1 in 0:1
                for y1 in 0:1
                        for (index,alpha) in enumerate(alpha_grid)
                                ...
                                constraint[counter]=@constraint(m,sum(psi[:,:,y1+1,:,:,:,x1+1,index])-temp*sum(psi[:,:,:,:,:,:,x1+1,index])==0)
                                counter+=1
                        end
                end
        end

        #Constraints for psi to be consistent with outcome distribution in period 2
        for y2 in 0:1
                for x2 in 0:1
                        for y1 in 0:1
                                for x1 in 0:1
                                        for (index,alpha) in enumerate(alpha_grid)
                                                temp = (softmax(theta*x2+alpha)^y2)*(1-softmax(theta*x2+alpha))^(1-y2)
                                                constraint[counter]=@constraint(m,sum(psi[:,y2+1,y1+1,:,:,x2+1,x1+1,index])-temp*sum(psi[:,:,y1+1,:,:,x2+1,x1+1,index])==0)
                                                counter+=1
                                        end
                                end
                        end
        ...
        #Constraints for psi to be consistent with outcome distribution in period 3
        for y3 in 0:1
                for x3 in 0:1
                        for y2 in 0:1
                                for x2 in 0:1
                                        for y1 in 0:1
                                                for x1 in 0:1
                                                        for (index,alpha) in enumerate(alpha_grid)
                                                                temp = (softmax(theta*x3+alpha)^y3)*(1-softmax(theta*x3+alpha))^(1-y3)
                                                                constraint[counter]=@constraint(m,sum(psi[y3+1,y2+1,y1+1,:,x3+1,x2+1,x1+1,index])-temp*sum(psi[:,y2+1,y1+1,:,x3+1,x2+1,x1+1,index])==0)
                                                                counter+=1
                                                        end
                                                end
                                        end
...
for theta in theta_grid
        ...
        #Order of dimensions: y3,y2,y1,x4,x3,x2,x1,alpha
        @variable(m_SE, psi[1:2,1:2,1:2,1:2,1:2,1:2,1:2,1:K])
                                        ...
                                        for x4 in 0:1
                                                for x3 in 0:1
                                                        for x2 in 0:1
                                                                for x1 in 0:1
                                                                        temp = map(alpha-> (softmax(theta*x4+alpha)^y4)*(1-softmax(theta*x4+alpha))^(1-y4),alpha_grid)
                                                                        constraint_SE[counter]=@constraint(m_SE, H[y4+1,y3+1,y2+1,y1+1,x4+1,x3+1,x2+1,x1+1]-temp'psi[y3+1,y2+1,y1+1,x4+1,x3+1,x2+1,x1+1,:]==0)
                                                                        counter+=1
                                                                end
                                                        end
                                                end
        ...
        for x1 in 0:1
                for y1 in 0:1
                        for (index,alpha) in enumerate(alpha_grid)
                                temp = (softmax(theta*x1+alpha)^y1)*(1-softmax(theta*x1+alpha))^(1-y1)
                                constraint_SE[counter]=@constraint(m_SE,sum(psi[:,:,y1+1,:,:,:,x1+1,index])-temp*sum(psi[:,:,:,:,:,:,x1+1,index])==0)
                                counter+=1
                        end
                end
        end
                ...
                for x2 in 0:1
                        for y1 in 0:1
                                for x1 in 0:1
                                        for (index,alpha) in enumerate(alpha_grid)
                                                temp = (softmax(theta*x2+alpha)^y2)*(1-softmax(theta*x2+alpha))^(1-y2)
                                                constraint_SE[counter]=@constraint(m_SE,sum(psi[:,y2+1,y1+1,:,:,x2+1,x1+1,index])-temp*sum(psi[:,:,y1+1,:,:,x2+1,x1+1,index])==0)
                                                counter+=1
                                        end
                                end
                        end
                                ...
                                for x2 in 0:1
                                        for y1 in 0:1
                                                for x1 in 0:1
                                                        for (index,alpha) in enumerate(alpha_grid)
                                                                temp = (softmax(theta*x3+alpha)^y3)*(1-softmax(theta*x3+alpha))^(1-y3)
                                                                constraint_SE[counter]=@constraint(m_SE,sum(psi[y3+1,y2+1,y1+1,:,x3+1,x2+1,x1+1,index])-temp*sum(psi[:,y2+1,y1+1,:,x3+1,x2+1,x1+1,index])==0)
                                                                counter+=1
                                                        end
                                                end
                                        end
                                        ...
                                        for (index,alpha) in enumerate(alpha_grid)
                                                for y3 in 0:1
                                                        for y2 in 0:1
                                                                for y1 in 0:1
                                                                        if y1+y2+y3!=3
                                                                                temp1 = softmax(theta*x3+alpha)*softmax(theta*x2+alpha)*softmax(theta*x1+alpha)
                                                                                temp2 = (softmax(theta*x3+alpha)^y3)*(1-softmax(theta*x3+alpha))^(1-y3)*(softmax(theta*x2+alpha)^y2)*(1-softmax(theta*x2+alpha))^(1-y2)*(softmax(theta*x1+alpha)^y1)*(1-softmax(theta*x1+alpha))^(1-y1)
                                                                                temp3 = temp1/temp2
                                                                                constraint_SE[counter]=@constraint(m_SE,psi[2,2,2,x4+1,x3+1,x2+1,x1+1,index]-temp3*psi[y3+1,y2+1,y1+1,x4+1,x3+1,x2+1,x1+1,index]==0)
                                                                                counter+=1
                                                                        end
        ...
        #3) Declare linear programming objective
        @objective(m_SE,Min,0.5*sum(psi[:,:,:,:,:,:,1,:])+0.5*sum(psi[:,:,:,:,:,:,2,:]))
        ...
        if string(termination_status(m_SE))=="OPTIMAL"
                ...
                #Upper bound for average partial effects associated to theta
                @objective(m_SE,Max,sum(map(k->0.5*sum(psi[:,:,:,:,:,:,:,k])*(softmax(theta+alpha_grid[k])-softmax(alpha_grid[k])),collect(1:K))))
                JuMP.optimize!(m_SE)
                push!(APE_set_max_SE,objective_value(m_SE))

                #Lower bound for average partial effects associated to theta
                @objective(m_SE,Min,sum(map(k->0.5*sum(psi[:,:,:,:,:,:,:,k])*(softmax(theta+alpha_grid[k])-softmax(alpha_grid[k])),collect(1:K))))
                JuMP.optimize!(m_SE)
...
draw(PDF(string(path_figures,"Identified_Set_APE_Logit_T4.pdf"), 6inch, 4inch), plot_identified_set_APE_true)
...
draw(PDF(string(path_figures,"Identified_Set_Logit_T4.pdf"), 6inch, 4inch), plot_identified_set_theta_true)
