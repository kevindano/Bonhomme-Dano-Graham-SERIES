#######################################################################################################
# Goal: we compare the true identified set for the probit case with T=2
...
for K_approx in [5,50,500]
        ...
        for y2 in 0:1
                for y1 in 0:1
                        for x2 in 0:1
                                for x1 in 0:1
                                temp = map(alpha-> (cdfnormal(theta*x2+alpha)^y2)*(1-cdfnormal(theta*x2+alpha))^(1-y2),alpha_grid_approx)
                                constraint[counter]=@constraint(m, H[y2+1,x2+1,y1+1,x1+1]-temp'psi[x2+1,y1+1,:,x1+1]==0)
                                counter+=1
                                end
                        end
        ...
        if string(termination_status(m))=="OPTIMAL"
                ...
                #Upper bound for average partial effects associated to theta
                @objective(m,Max,sum(map(k->0.5*sum(psi[:,:,k,:])*(cdfnormal(theta+alpha_grid_approx[k])-cdfnormal(alpha_grid_approx[k])),collect(1:K_approx))))
                JuMP.optimize!(m)
                push!(APE_set_max_approx,objective_value(m))

                #Lower bound for average partial effects associated to theta
                @objective(m,Min,sum(map(k->0.5*sum(psi[:,:,k,:])*(cdfnormal(theta+alpha_grid_approx[k])-cdfnormal(alpha_grid_approx[k])),collect(1:K_approx))))
                JuMP.optimize!(m)
...
draw(PDF(string(path_figures,"Identified_Set_Probit_T2_Predetermined_K31_Kapprox5.pdf"), 6inch, 4inch), plot_identified_set_theta_true_K31_Kapprox5)
...
draw(PDF(string(path_figures,"Identified_Set_Probit_T2_Predetermined_K31_Kapprox50.pdf"), 6inch, 4inch), plot_identified_set_theta_true_K31_Kapprox50)
...
draw(PDF(string(path_figures,"Identified_Set_Probit_T2_Predetermined_K31_Kapprox500.pdf"), 6inch, 4inch), plot_identified_set_theta_true_K31_Kapprox500)
...
draw(PDF(string(path_figures,"Identified_Set_APE_Probit_T2_Predetermined_K31_Kapprox5.pdf"), 6inch, 4inch), plot_identified_set_APE_true_K31_Kapprox5)
...
draw(PDF(string(path_figures,"Identified_Set_APE_Probit_T2_Predetermined_K31_Kapprox50.pdf"), 6inch, 4inch), plot_identified_set_APE_true_K31_Kapprox50)
...
draw(PDF(string(path_figures,"Identified_Set_APE_Probit_T2_Predetermined_K31_Kapprox500.pdf"), 6inch, 4inch), plot_identified_set_APE_true_K31_Kapprox500)
