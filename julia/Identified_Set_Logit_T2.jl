########################################################################
# Goal: computation of the identified set for the logit case with T=2
# Julia Version 1.5.3
########################################################################
...
for (index,theta_true) in enumerate(theta_true_grid)
        ...
        for theta in theta_grid
                ...
                for x2 in 0:1
                        for x1 in 0:1
                                for (index,alpha) in enumerate(alpha_grid)
                                        constraint_SE[counter]=@constraint(m_SE,psi[x2+1,2,index,x1+1]*(1-softmax(theta*x1+alpha)) - psi[x2+1,1,index,x1+1]*softmax(theta*x1+alpha)== 0)
                                        counter+=1
                                end
                        end
                end
...
draw(PDF(string(path_figures,"Identified_Set_APE_Logit_T2.pdf"), 6inch, 4inch), plot_identified_set_APE_true)
...
draw(PDF(string(path_figures,"Identified_Set_Logit_T2.pdf"), 6inch, 4inch), plot_identified_set_theta_true)
