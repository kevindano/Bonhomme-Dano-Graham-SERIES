########################################################################
# Goal: computation of the identified set for the probit case with T=3
# Julia Version 1.5.3
########################################################################
...
function true_conditional_probability_given_alpha_x1(theta,y3,y2,y1,x3,x2,x1,alpha)
        ...
        H_alpha = (cdfnormal(theta*x3+alpha)^y3)*((1-cdfnormal(theta*x3+alpha))^(1-y3))*(1/2)
                ...
                for y2 in 0:1
                        for y1 in 0:1
                                for x3 in 0:1
                                        for x2 in 0:1
                                                for x1 in 0:1
                                                        temp = map(alpha-> (cdfnormal(theta*x3+alpha)^y3)*(1-cdfnormal(theta*x3+alpha))^(1-y3),alpha_grid)
                        ...
                        for x1 in 0:1
                                for (index,alpha) in enumerate(alpha_grid)
                                        for y2 in 0:1
                                                for y1 in 0:1
                                                        if y1+y2!=2
                                                                temp1 = cdfnormal(theta*x2+alpha)*cdfnormal(theta*x1+alpha)
                                                                temp2 = (cdfnormal(theta*x2+alpha)^y2)*(1-cdfnormal(theta*x2+alpha))^(1-y2)*(cdfnormal(theta*x1+alpha)^y1)*(1-cdfnormal(theta*x1+alpha))^(1-y1)
                                                                temp3 = temp1/temp2
...
draw(PDF(string(path_figures,"Identified_Set_APE_Probit_T3.pdf"), 6inch, 4inch), plot_identified_set_APE_true)
...
draw(PDF(string(path_figures,"Identified_Set_Probit_T3.pdf"), 6inch, 4inch), plot_identified_set_theta_true)
