########################################################################
# Goal: computation of the identified set for the logit case with T=3
# Julia Version 1.5.3
########################################################################
...
function true_conditional_probability_given_alpha_x1(theta,y3,y2,y1,x3,x2,x1,alpha)
        #covariates X are iid Bernouilli(1/2) independent of everything else
        H_alpha = (softmax(theta*x3+alpha)^y3)*((1-softmax(theta*x3+alpha))^(1-y3))*(1/2)
...
function true_conditional_probability_given_x1(theta,y3,y2,y1,x3,x2,x1,alpha_grid,pi_alpha)
        H = 0.0
        for (index,alpha) in enumerate(alpha_grid)
                H_alpha = true_conditional_probability_given_alpha_x1(theta,y3,y2,y1,x3,x2,x1,alpha)
...
function true_probabilities(theta,alpha_grid,pi_alpha)
        H = zeros(2,2,2,2,2,2) #order y3,y2,y1,x3,x2,x1 - 1st entry is for "0" and 2nd is for "1"
        for y3 in 0:1
                for x3 in 0:1
                        for y2 in 0:1
                                for x2 in 0:1
                                        for y1 in 0:1
                                                for x1 in 0:1
                                                        H[y3+1,y2+1,y1+1,x3+1,x2+1,x1+1]=true_conditional_probability_given_x1(theta,y3,y2,y1,x3,x2,x1,alpha_grid,pi_alpha)
                                                end
                                        end
                                end
                        end
...
for theta in theta_grid
        ...
        #Order of dimensions: y2,y1,x3,x2,x1,alpha
        @variable(m, psi[1:2,1:2,1:2,1:2,1:2,1:K])
                ...
                for y2 in 0:1
                        for y1 in 0:1
                                for x3 in 0:1
                                        for x2 in 0:1
                                                for x1 in 0:1
                                                        temp = map(alpha-> (softmax(theta*x3+alpha)^y3)*(1-softmax(theta*x3+alpha))^(1-y3),alpha_grid)
                                                        constraint[counter]=@constraint(m, H[y3+1,y2+1,y1+1,x3+1,x2+1,x1+1]-temp'psi[y2+1,y1+1,x3+1,x2+1,x1+1,:]==0)
                                                        counter+=1
                                                end
                                        end
                                end
...
for theta in theta_grid
        ...
        #Order of dimensions: y2,y1,x3,x2,x1,alpha
        @variable(m_SE, psi[1:2,1:2,1:2,1:2,1:2,1:K])
                        ...
                        for y1 in 0:1
                                for x3 in 0:1
                                        for x2 in 0:1
                                                for x1 in 0:1
                                                        temp = map(alpha-> (softmax(theta*x3+alpha)^y3)*(1-softmax(theta*x3+alpha))^(1-y3),alpha_grid)
                                                        constraint_SE[counter]=@constraint(m_SE, H[y3+1,y2+1,y1+1,x3+1,x2+1,x1+1]-temp'psi[y2+1,y1+1,x3+1,x2+1,x1+1,:]==0)
                                                        counter+=1
                                                end
                                        end
                                end
                        ...
                        for x1 in 0:1
                                for (index,alpha) in enumerate(alpha_grid)
                                        for y2 in 0:1
                                                for y1 in 0:1
                                                        if y1+y2!=2
                                                                temp1 = softmax(theta*x2+alpha)*softmax(theta*x1+alpha)
                                                                temp2 = (softmax(theta*x2+alpha)^y2)*(1-softmax(theta*x2+alpha))^(1-y2)*(softmax(theta*x1+alpha)^y1)*(1-softmax(theta*x1+alpha))^(1-y1)
                                                                temp3 = temp1/temp2
                                                                constraint_SE[counter]=@constraint(m_SE,psi[2,2,x3+1,x2+1,x1+1,index]-temp3*psi[y2+1,y1+1,x3+1,x2+1,x1+1,index]==0)
                                                                counter+=1
                                                        end
...
draw(PDF(string(path_figures,"Identified_Set_APE_Logit_T3.pdf"), 6inch, 4inch), plot_identified_set_APE_true)
...
draw(PDF(string(path_figures,"Identified_Set_Logit_T3.pdf"), 6inch, 4inch), plot_identified_set_theta_true)
